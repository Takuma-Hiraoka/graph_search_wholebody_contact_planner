    ...
    friend bool operator==(const ContactState& a, const ContactState& b) { // ContactStateはlocalPoseのrotationの違いのみ無視する. rotationは接触時にZ方向を無視するため. graphとして再訪を防ぐために比較する.
      ...
      for (int i=0;i<a.contacts.size();i++) {
        ...
        for (int j=0;(j<b.contacts.size()) && !has_a; j++) {
          ...
          if (!((a.contacts[i].c1.linkName == b.contacts[j].c1.linkName) && (a.contacts[i].c1.localPose.translation() == b.contacts[j].c1.localPose.translation()) && (a.contacts[i].c2.linkName == b.contacts[j].c2.linkName) && (a.contacts[i].c2.localPose.translation() == b.contacts[j].c2.localPose.translation()) ||
                ((a.contacts[i].c1.linkName == b.contacts[j].c2.linkName) && (a.contacts[i].c1.localPose.translation() == b.contacts[j].c2.localPose.translation()) && (a.contacts[i].c2.linkName == b.contacts[j].c1.linkName) && (a.contacts[i].c2.localPose.translation() == b.contacts[j].c1.localPose.translation())))) continue; // translationが違う
          has_a = true;
        }
